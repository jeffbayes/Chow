### This voodoo magic is supposed to talk to the Locu Parser.
...
def do_something_cool(search_term):
    t = time.strptime("Monday 12:00:00", "%A %H:%M:%S")
    venue_client = VenueApiClient(KEY)
    menu_item_client = MenuItemApiClient(KEY)
    menu_items = menu_item_client.search(locality = 'Eugene', name = search_term)  
    item = menu_items['objects'][0]
    dish = Dish(item,t)
    venueID = item["venue"]["id"]
    print("Name: " + dish.get_attr('name'))
    print("Description: " + dish.get_attr("description"))
    print("Price: " + str(dish.get_attr("price")))
    print("Venue: " + dish.get_attr("venue"))
    
    
do_something_cool('pizza')
